...
const TermsStep = ({
  ...
  const getInitialValues = (localUser: TUser) => {
    ...
    if (localUser?.understand_disclaimer || kcContext.userProfile.understand_disclaimer) {
  ...
  return (
    <Space direction="vertical" size={24} className={styles.termsAndConditionsWrapper}>
      ...
      <Text>{advancedMsg('portal_introduction')}</Text>
      <GridCard
        wrapperClassName={styles.cardWrapper}
        className={styles.card}
        title={
          <div className={styles.termsCardHeader}>
            <span className={styles.title}>{advancedMsg('terms_title')}</span>
            <span className={styles.lastUpdateDate}>
              {LAST_UPDATED_DATE}
              <Tooltip
                arrowPointAtCenter
                placement="top"
                title={advancedMsg('last_update_tooltip')}
                className={styles.tooltipGold}
              >
          ...
          rules={[
            {
              validator: async (rule, value: string[]) =>
                value?.length === 2
                  ? Promise.resolve()
                  : Promise.reject(advancedMsg('terms_disclaimer_error')),
      ...
      <Space className={styles.checkboxFormFooter}>
        ...
        <Button type="primary" className="iconRight" onClick={() => form.submit()}>
